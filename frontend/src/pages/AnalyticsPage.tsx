...
import { useInnovex } from "@/hooks/useCompanies";
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    const fetchFullCompanies = async () => {
      try {
        ...
        const parsed = data.map(row => row.full_json as CompanyFull).filter(Boolean);
        setCompanies(parsed);
      } catch (error) {
        console.error("Error fetching full companies:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const tierData = useMemo(() => {
    const counts: Record<string, number> = {};
    companies.forEach((c) => {
      const tier = c.intelligence_data?.company_tier || "Unclassified";
  ...
  const layoffsData = useMemo(() => {
    ...
    companies.forEach((c) => {
      const events = c.layoffs_data?.layoff_events || [];
      events.forEach(e => {
        if (e.year >= 2024) {
          years[e.year] = (years[e.year] || 0) + (e.number_laid_off || 0);
        }
      });
    });
    return Object.entries(years).map(([year, total]) => ({ year, total })).sort((a, b) => Number(a.year) - Number(b.year));
  }, [companies]);

  const effortData = useMemo(() => {
    let easy = 0, medium = 0, hard = 0, extreme = 0;
    companies.forEach((c) => {
      const diff = (c.roadmap_data?.difficulty || "").toLowerCase();
      if (diff.includes("easy")) easy++;
      else if (diff.includes("medium")) medium++;
      else if (diff.includes("hard")) hard++;
      else if (diff.includes("extreme")) extreme++;
    });
    return [
        ...
        {selectedId === "all" ? (
          <div className="animate-fade-in">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="surface-card p-6">
                <h3 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-6">Tier Classification</h3>
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
            ...
            {/* Added: Layoffs Trend & Skill Effort */}
            <div className="grid md:grid-cols-2 gap-6 mt-6">
              <div className="surface-card p-6">
                <h3 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-6">Layoffs Impact (2024-2025)</h3>
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={layoffsData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
                      ...
                      <Tooltip 
                        ...
                        contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: '8px', color: '#f1f5f9' }}
                      />
                      <Bar dataKey="total" fill="#ef4444" radius={[4, 4, 0, 0]} name="Employees Laid Off" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="surface-card p-6">
                <h3 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-6">Avg Skills & Effort Required</h3>
