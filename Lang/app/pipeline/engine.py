...
async def fetch_chunk(
    ...
    try:
        ...
        schema_chunk = "\n".join(schema_lines)
        prompt = RESEARCH_PROMPT_TEMPLATE.format(company_name=company, schema_chunk=schema_chunk)
...
async def run_parallel_research_engine(company_name: str, params_to_research: List[str]) -> Dict[str, Any]:
    """
    ...
    provider_configs = [
        ...
        {"name": "grok", "model": settings.DEFAULT_GROK_MODEL, "fallback": settings.FALLBACK_GROK_MODEL},
        {"name": "groq", "model": settings.DEFAULT_GROQ_MODEL, "fallback": settings.FALLBACK_GROQ_MODEL},
    ]
